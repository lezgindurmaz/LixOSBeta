...
static long sf_ctl_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) {
        ...
        case SF_IOC_SET_SPI_BUF_SIZE: {
            if (arg == 0 || arg > 1024 * 1024) { // 1MB limit
                err = -EINVAL;
                break;
            }
...
{
    int len = 0;
    len += scnprintf((char *)buf, PAGE_SIZE, "chip   : %s %s\nid     : 0x0 lib:%s\nvendor : fw:%s\nmore   : fingerprint\n",
...
{
    int ret = 0;
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "solution:%s\n", sf_hw_ver.tee_solution);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "ca      :%s\n", sf_hw_ver.ca_version);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "ta      :%s\n", sf_hw_ver.ta_version);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "alg     :%s\n", sf_hw_ver.algorithm);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "nav     :%s\n", sf_hw_ver.algo_nav);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "driver  :%s\n", sf_hw_ver.driver);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "firmware:%s\n", sf_hw_ver.firmware);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "sensor  :%s\n", sf_hw_ver.fortsense_id);
    ret += scnprintf(buf + ret, PAGE_SIZE - ret, "vendor  :%s\n", sf_hw_ver.vendor_id);
    return ret;
}
                 ...
                 const char *buf, size_t count)
{
    ...
    // fb_blank(NULL, blank); // Removed to prevent NULL pointer dereference
