		...
		unsigned long arg)
{
	...
	if (ret) {
		MI_TOUCH_LOGE(1, "%s %s: copy_from_user failed\n", MI_TAG, __func__);
	...
	switch (user_cmd) {
	case SET_CUR_VALUE:
		if (touch_data->ops && touch_data->ops->setModeValue) {
			if (buf[0] == Touch_Debug_Level  && mi_log_level != TOUCH_DEBUG) {
				...
				buf[0] = touch_data->ops->setModeValue(buf[0], buf[1]);
			}
		}
		break;
	case GET_CUR_VALUE:
	...
	case GET_MIN_VALUE:
	case GET_MAX_VALUE:
		if (touch_data->ops && touch_data->ops->getModeValue)
			buf[0] = touch_data->ops->getModeValue(buf[0], user_cmd);
		break;
	case RESET_MODE:
		if (touch_data->ops && touch_data->ops->resetMode)
			buf[0] = touch_data->ops->resetMode(buf[0]);
		break;
	case GET_MODE_VALUE:
		if (touch_data->ops && touch_data->ops->getModeAll)
			ret = touch_data->ops->getModeAll(buf[0], buf);
		break;
	case SET_LONG_VALUE:
		if (touch_data->ops && touch_data->ops->setModeLongValue && buf[1] > 0 && buf[1] <= MAX_BUF_SIZE - 2)
			ret = touch_data->ops->setModeLongValue(buf[0], buf[1], &buf[2]);
		else
			ret = -EINVAL;
		break;
	default:
...
#ifdef CONFIG_COMPAT
static long xiaomi_touch_dev_compat_ioctl(struct file *file, unsigned int cmd,
					unsigned long arg)
{
	return xiaomi_touch_dev_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
}
#endif
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = xiaomi_touch_dev_compat_ioctl,
#else
...
int xiaomitouch_register_modedata(struct xiaomi_touch_interface *data)
{
	...
	if (!touch_data->ops) {
		touch_data->ops = kzalloc(sizeof(struct xiaomi_touch_ops), GFP_KERNEL);
		if (!touch_data->ops) {
	...
	touch_data->ops->setModeValue = data->ops->setModeValue;
	touch_data->ops->getModeValue = data->ops->getModeValue;
	touch_data->ops->resetMode = data->ops->resetMode;
	touch_data->ops->getModeAll = data->ops->getModeAll;
	touch_data->ops->palm_sensor_read = data->ops->palm_sensor_read;
	touch_data->ops->palm_sensor_write = data->ops->palm_sensor_write;
	touch_data->ops->p_sensor_read = data->ops->p_sensor_read;
	touch_data->ops->p_sensor_write = data->ops->p_sensor_write;
	touch_data->ops->touch_vendor_read = data->ops->touch_vendor_read;
	touch_data->ops->setModeLongValue = data->ops->setModeLongValue;
	touch_data->ops->panel_color_read = data->ops->panel_color_read;
	touch_data->ops->panel_display_read = data->ops->panel_display_read;
	touch_data->ops->panel_vendor_read = data->ops->panel_vendor_read;
...
struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (pdata->touch_data->ops && pdata->touch_data->ops->palm_sensor_write)
		pdata->touch_data->ops->palm_sensor_write(!!input);
	else {
...
struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (pdata->touch_data->ops && pdata->touch_data->ops->p_sensor_write)
		pdata->touch_data->ops->p_sensor_write(!!input);
	else {
...
static ssize_t xiaomi_touch_vendor_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char value = '0';
	struct xiaomi_touch_pdata *pdata = dev_get_drvdata(dev);
	if (pdata->touch_data->ops && pdata->touch_data->ops->touch_vendor_read)
		value = pdata->touch_data->ops->touch_vendor_read();
	else
...
static ssize_t xiaomi_panel_color_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char value = '0';
	struct xiaomi_touch_pdata *pdata = dev_get_drvdata(dev);
	if (pdata->touch_data->ops && pdata->touch_data->ops->panel_color_read)
		value = pdata->touch_data->ops->panel_color_read();
	else
...
static ssize_t xiaomi_panel_vendor_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char value = '0';
	struct xiaomi_touch_pdata *pdata = dev_get_drvdata(dev);
	if (pdata->touch_data->ops && pdata->touch_data->ops->panel_vendor_read)
		value = pdata->touch_data->ops->panel_vendor_read();
	else
		MI_TOUCH_LOGE(1, "Touch vendor not define,return default value!");
	if (value == '0')
		return 0;
...
static ssize_t xiaomi_panel_display_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	char value = '0';
	struct xiaomi_touch_pdata *pdata = dev_get_drvdata(dev);
	if (pdata->touch_data->ops && pdata->touch_data->ops->panel_display_read)
		value = pdata->touch_data->ops->panel_display_read();
	else
		MI_TOUCH_LOGE(1, "Touch vendor not define,return default value!");
	if (value == '0')
		return 0;
