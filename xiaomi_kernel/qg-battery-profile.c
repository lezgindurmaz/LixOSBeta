						...
						unsigned long arg)
{
	...
	case BPIOCXFCC:
		...
		break;
	case BPIOCXVAR:
		if (bp.table_index < TABLE_Z1 || bp.table_index > TABLE_Z6) {
...
#ifdef CONFIG_COMPAT
static long qg_battery_data_compat_ioctl(struct file *file, unsigned int cmd,
					unsigned long arg)
{
	return qg_battery_data_ioctl(file, cmd, (unsigned long)compat_ptr(arg));
}
#endif
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = qg_battery_data_compat_ioctl,
#else
